...
type Dd interface {
	...
	AllCategory() ([]Category, error)                        // 获取所有分类
	...
	GetClass(sourceId uint) ([]Class, error)                 // 通过采集源id获取其下所有采集类
	...
	SearchContent(keyword string, num int, pg int) ([]Content, int, error)                           // 全局搜索影片
	SearchContent_Category(categoryId uint, keyword string, num int, pg int) ([]Content, int, error) // 搜索自建分类下影片
	SearchContent_Class(classId uint, keyword string, num int, pg int) ([]Content, int, error)       // 搜索采集分类下影片
	SearchContent_Source(sourceId uint, keyword string, num int, pg int) ([]Content, int, error)     // 搜索某个采集源下的影片
	ContentList(num int, pg int) ([]Content, int, error)                                             // 全局影片列表
	ContentList_Category(categoryId uint, num int, pg int) ([]Content, int, error)                   // 自建分类影片列表
	ContentList_Class(classId uint, num int, pg int) ([]Content, int, error)                         // 采集类影片列表
	ContentList_Source(sourceId uint, num int, pg int) ([]Content, int, error)                       // 采集源影片列表
	GetContent(id uint) (Content, error)                                                             // 获取影片
	BrowseContentByCategory(categoryId uint, num int, pg int) ([]Content, int, error)                // 分类下的影片
	...
	AllSource() ([]Source, error)                          // 获取所有采集源
